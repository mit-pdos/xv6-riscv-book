...
\lstinline{PTE_V}\index{PTE_V@\lstinline{PTE_V}}
indicates whether the PTE is present: if it is
not set, a reference to the page causes a fault (i.e. is not allowed).
\lstinline{PTE_R}\index{PTE_R@\lstinline{PTE_R}}
controls whether instructions are allowed to issue
reads to the page.
\lstinline{PTE_W}\index{PTE_W@\lstinline{PTE_W}}
controls whether instructions are allowed to issue
writes to the page.
\lstinline{PTE_X}\index{PTE_X@\lstinline{PTE_X}}
...
\lstinline{PHYSTOP}\index{PHYSTOP@\lstinline{PHYSTOP}},
...
\lstinline{mappages}
calls
\lstinline{walk}\index{walk@\lstinline{walk}}
...
\lstinline{main}\index{main@\lstinline{main}}
calls
\lstinline{kvminithart}\index{kvminithart@\lstinline{kvminithart}}
...
\lstinline{struct run}\index{struct run@\lstinline{struct run}}
...
\lstinline{main}\index{main@\lstinline{main}}
calls
\lstinline{kinit}\index{kinit@\lstinline{kinit}}
...
\lstinline{r}.
\lstinline{kalloc}\index{kalloc@\lstinline{kalloc}}
...
calls
\lstinline{sbrk}\index{sbrk@\lstinline{sbrk}}.
...
\lstinline{struct elfhdr}\index{struct elfhdr@\lstinline{struct elfhdr}}
...
.code 'F',
or
\lstinline{ELF_MAGIC}\index{ELF_MAGIC@\lstinline{ELF_MAGIC}}
...
\lstinline{proc_pagetable}\index{proc_pagetable@\lstinline{proc_pagetable}}
\lineref{kernel/exec.c:/proc_pagetable/},
allocates memory for each ELF segment with
\lstinline{uvmalloc}\index{uvmalloc@\lstinline{uvmalloc}}
\lineref{kernel/exec.c:/uvmalloc/},
and loads each segment into memory with
\lstinline{loadseg}\index{loadseg@\lstinline{loadseg}}
\lineref{kernel/exec.c:/loadseg/}.
\lstinline{loadseg}
uses
\lstinline{walkaddr}\index{walkaddr@\lstinline{walkaddr}}
...
\lstinline{/init}\index{/init@\lstinline{/init}},
...
\lstinline{ustack}\index{ustack@\lstinline{ustack}}.
It places a null pointer at the end of what will be the
\lstinline{argv}\index{argv@\lstinline{argv}}
list passed to
\lstinline{main}.
The first three entries in
\lstinline{ustack}
are the fake return PC,
\lstinline{argc}\index{argc@\lstinline{argc}},
and
\lstinline{argv}
pointer.
...
in that situation,
the
\lstinline{copyout}\index{copyout@\lstinline{copyout}}
