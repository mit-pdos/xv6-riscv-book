...
\item The kernel stack pages.  Each process has its own kernel stack,
  which is mapped high so that below it xv6 can map a \indextext{guard
    page}.  The guard page is not mapped and it ensures that if the
  kernel runs of a kernel stack, it will cause a fault.  Without a
  guard page the stack could grow down into kernel data, which could
  result in strange bugs that are difficult to track down.
