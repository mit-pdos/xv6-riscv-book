...
determine what memory addresses mean.  They allow xv6 to multiplex
...
physical address whose top 44 vits come from the PPN in the PTE
...
4096-byte page that contains 512 PTEs, which contain the physical
addresses for pages in the next level of the tree.  Each of those
...
write the physical address of the root page into the 
...
Virtual memory refers to the collection of abstractions and mechanisms
...
\item The kernel stack pages.  Each process has its own kernel stack,
  which is mapped high so that below it xv6 can map a \indextext{guard
    page}.  The guard page's mapping is invalid (i.e.,
...
The kernel stacks themselves are located in kernel memory and also
