...
Similarly, by mapping data without \lstinline{PTE_U}, a user program
...
\lstinline{PTE_V}), thus a single page of physical memory shows up in
...
\section{Code: exec}
%% 
\lstinline{Exec} is the system call that creates a user address space.
It initializes the address space from a file stored in the file
system.  \lstinline{Exec} \lineref{kernel/exec.c:/^exec/} opens the
named binary \lstinline{path} using \indexcode{namei}
\lineref{kernel/exec.c:/namei/}, which is explained in
Chapter~\ref{CH:FS}.  Then, it reads the ELF header. Xv6 applications
are described in the widely-used \indextext{ELF format}, defined in
...
in memory (without write permissions) and that its content at offset
