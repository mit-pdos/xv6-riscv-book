...
(i.e., are on disk),
it can call the \indexcode{sync} \lineref{kernel/log.c:/sys_sync/} system call after a write system call;
\lstinline{sync} waits until an in-progress group-commit has committed.
...
A key assumption in the implementation of \lstinline{end_op} is that
\lstinline{write_head} updates the header block atomically: completely or
not at all.
Recall from Chapter~\ref{CH:FS} that a block in xv6 is two disk sectors, but the log header
...
free to add more \lstinline{printk} statements to help you answering
the question.
