...
respond in much more interesting ways. For
instance, many kernels use page faults to implement
virtual memory
features such as \indextext{copy-on-write (COW) fork}.
...
parent's. It would be nice if the child and parent could share 
physical memory, instead of having the parent making a copy of its 
address space using \lstinline{uvmcopy}
\lineref{kernel/vm.c:/^uvmcopy/}.  The kernel, however, cannot
unconditionally share physical memory between parent and child,
since they would then see each other's memory writes; that would
rapidly cause them to malfunction.

Parent and child can safely share phyical memory
using page faults and COW fork.  When a
...
child will experience few page faults, and the kernel can avoid making
a complete copy.  Furthermore, this features is completely transparent to the
...
it. First, when an application calls \lstinline{sbrk}, the kernel grows
...
the kernel allocates the physical
