...
While commonality among the three trap types suggests that a kernel
...
separate paths for three cases: traps from kernel space, traps from
user space, and timer interrupts. In each case, trap handling
has four parts: hardware actions taken by the RISC-V CPU,
an assembly ``vector'' that prepares the way for kernel C code,
a C trap handler that decides what to do with the trap,
and the system call or device service routine.
...
RISC-V supports a number of control registers that the kernel uses to
tell the CPU how to handle interrupts, and that the kernel can query
...
When it needs to raise a trap, the RISC-V hardware does the
following:

\begin{enumerate}

\item If the trap is a device interrupt, and the {\tt sstatus} SIE bit
  is clear, don't do anything.

\item Copy the {\tt pc} to {\tt pc}.

\item Save the current mode in the SPP bit in {\tt sstatus}.
...
\item Start executing.

\end{enumerate}

It is important that the CPU performs all these steps atomically.
...
Note that the CPU doesn't switch page tables or stacks on a
...
\section{Low-level kernel trap handling}
...
software. The first stage of trap handling is done in assembly code to
set up an environment in which it is safe to run C code, and then the
assembly code jumps to C code that figures how how to react to the
trap. There are two main cases (again ignoring timer interrupts):
...
\item Traps from supervisor mode. These are traps that occur while the
  ...
  assembly handler code needs to push the current registers onto the
  stack, and jump to C code.
