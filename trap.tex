...
Kernel code that implements system calls such as {\tt exec} needs to
be able to find the arguments passed by user code. User code calls
wrapper functions in order to make system calls. This causes system
call arguments to be where the RISC-V C calling convention places
them: in registers.

In the kernel, system call arguments are available in the
saved registers in the trapframe, {\tt p->trapframe}.
The functions
\lstinline{argint},
\lstinline{argaddr},
and
