...
kernel be allowed to use devices, and because only the kernel has the
state needed for device handling. It also makes sense for exceptions
...
% point out that the RISC-V doesn't help us with the stack pointer either
...
trap. This means that the address in {\tt stvec} must have a valid
...
can continue to execute even after it switches to the kernel page
table.
