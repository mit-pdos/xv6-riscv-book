...
{\tt satp} points to a user page table that has no mappings for the
...
{\tt uservec} now proceeds to save all user registers there,
including (after another swap) the user's {\tt a0}.

The {\tt trapframe} also contains a pointer to the current process's
...
and calls {\tt usertrap} \lineref{kernel/trap.c:/^usertrap/}.

XXX usertrap...

XXX where is the trampoline stuff set up.
