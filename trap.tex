...
% I'm using "trap" to include system calls, exceptions, and
% device interrupts. The xv6 source does the same
% (e.g. trap.c handles all three), as does the RISC-V
% documentation (the stvec is supervisor trap vector,
% and handles all three). I'm using "interrupt" to refer
% just to device interrupts.
...
software.   RISC-V has registers (\indexcode{mtvec} and
\indexcode{stvec}) to specify the address of the \indextext{trap
  handler} in machine mode or supervisor mode, respectively.
It also
has registers (\indexcode{mie} and \indexcode{sie}) to specify which
