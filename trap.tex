...
\item \indexcode{sstatus}: The SIE bit controls whether device interrupts
...
\textit{n} 'th 
system call
as either an integer, pointer, or a file descriptor.
...
Xv6's response to exceptions is quite boring: if the exception happens
in user space, the kernel kills the faulting process.  If the
exceptions happens in the kernel, the kernel panics.  Real operating,
however, respond in much more interesting ways to exceptions. For
instance, kernels use exceptions due to page faults to implement
widely-used features, such as \indextext{copy-on-write (COW) fork}.

To explain copy-on-write fork, consider xv6's \lstinline{fork}. The
parent creates a new address space for the child as described in
Chapter~\ref{CH:MEM}. This address space is identical to the
parent's. It would be nice if the child and parent could share the
address space, instead of having the parent making a copy of its own
address space using \lstinline{uvmcopy}
\lineref{kernel/vm.c:/^uvmcopy/}.  The kernel, however, cannot
unconditionally share the address space between parent and child
because the parent and child may write to the address space and, if
the address space is shared, those writes will change the other
process; that breaks isolation.

This challenge can be solved using page faults and COW fork.  When a
...
different page faults: load page fault (when a load instruction cannot
translate its virtual address), a store page fault (when a store
instruction cannot translate its virtual address), and an instruction
page fault (when the address for instruction doesn't translate).  The
...
The basic plan in COW fork is for the parent and child to share all
pages but mapped read-only.  Thus, when the child or parent executes a
...
child's address space and one read/write in the parent's address
...
kernel has updated the page tables, the faulting instruction will now
...
child will experience no page faults, and the kernel can avoid making
a copy.  Furthermore, this features is completely transparent to the
applications: no modifications to applications are necessary for
applications to benefit from COW fork.
...
feature is called \indextext{lazy allocation}, which has two parts to
it. First, when application calls \lstinline{sbrk}, the kernel grows
the address space, but marks the new addresses as not valid in the
page table. Second, on a page fault, the kernel allocates the physical
...
allocates memory only when the application is actually using it.  Like
COW-fork, the kernel can implement this feature transparent to
applications.

Yes another widely-used feature that combines paging and page faults
is \indextext{paging from disk}.  If several applications use more
memory than fits in physical memory, the kernel can move some
application pages to a storage device such as a disk and marking their
entries in the page table as not valid. If an application uses an
...
that is on disk, the kernel fetches the page from disk, marks the page
as present, and resumes the application. To make room for the page,
...
of reference (i.e., they don't use all their memory constantly).
