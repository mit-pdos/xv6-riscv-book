...
The RISC-V processor on which xv6 is running has a \textit{Core Local
...
itself.  The RISC-V processor has registers (\indexcode{mtvec} and
...
processor to kernel mode and change the program counter to a
...
mode: switch page tables, switch stacks, etc.

To return from an interrupt to the original mode, the RISC-V processor
...
programs the processor to delegate all machine-mode interrupts to
...
\item Interrupts in user mode. This case is challenging.  At the time
  ...
  every process's address space and in the kernel address space (as we
  saw in Figure~\ref{fig:as} and in Figure~\ref{fig:xv6_layout}).
  This page is mapped at the same address in each address space.  This
  page contains the code that switches page tables, and because the
  page is mapped identically in each page table, the instruction right
  after the page table switch will translate identically.  The
  trampoline also switches from a user stack to a valid kernel stack.
...
to save them, it needs a register.  To handle this conundrum, the
RISC-V processor provides a scratch register for machine and kernel
mode: \indexcode{mscratch} and \indexcode{sscratch}, respectively.
The RISC-V processor also has an instruction to exchange the content
...
interrupt happened, the RISC-V processor provides a register
...
the processor will translate virtual address with the user page table;
...
process's trapframe \lstinline{p->tf}.  The kernel stores the new
...
back in the kernel with a kernel page table and on a kernel stack,
running C code.
...
are delivered to another processor.  If some network connections are
short lived while others are long lived and the operating system wants
to keep all processors busy to achieve high throughput, this routing
