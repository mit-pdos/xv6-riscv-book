...
{\tt sleep} call
\lineref{kernel/console.c:/^......sleep..;/}
...
before sending each byte, {\tt uartwrite} reads the UART's
LSR register to see whether the UART can accept another byte,
and calls {\tt sleep} if it cannot;
the interrupt routine calls {\tt wakeup} when the UART signals
it is ready for more output, which causes {\tt uartwrite}
to wake up, re-check the LSR register, and send the next byte.
...
contains a time at which the CPU will raise a timer
