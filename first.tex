...
\textit{time-share}\index{time-share} 
...
\textit{isolation}\index{isolation} 
...
\textit{monolithic kernel}\index{monolithic kernel}, 
...
\textit{machine mode}\index{machine mode},
\textit{kernel mode}\index{kernel mode} 
(called
\textit{supervisor mode}\index{supervisor mode}
...
\textit{privileged instructions}\index{privileged instructions}.
...
\textit{microkernel}\index{microkernel}.
...
\textit{address space}\index{address space}, 
...
\textit{virtual address}\index{virtual address}
(the address that an RISC-V instruction manipulates) to a
\textit{physical address}\index{physical address}
...
\textit{user memory}\index{user memory}
...
\textit{trampoline}\index{trampoline}
...
\lstinline{p->xxx}\index{p->xxx@\lstinline{p->xxx}}
to refer to elements of the
\lstinline{proc}
structure.

Each process has a thread of execution (or 
\textit{thread}\index{thread}
...
\lstinline{p->state}\index{p->state@\lstinline{p->state}} 
indicates whether the process is allocated, ready
to run, running, waiting for I/O, or exiting.

\lstinline{p->pagetable}\index{p->pagetable@\lstinline{p->pagetable}}
...
\lstinline{start}\index{start@\lstinline{start}}
...
\lineref{kernel/kernelvec.S:/^machinevec/}
