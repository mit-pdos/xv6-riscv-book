...
computation with the term \indextext{CPU}.  Other documentation (e.g.,
RISC-V specifications) also uses the words processor, core, and hart
instead of CPU.  Related terms are \indextext{multicore processor} (a
single chip with
several CPUs on it) and \indextext{multiprocessor} (a computer board with several
processor chips).
