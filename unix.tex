...
\lstinline{user/sh.c} \fileref{user/sh.c}. The \textcolor{blue}{link} is a hyperlink to
the relevant xv6 source code at
\url{https://github.com/mit-pdos/xv6-riscv/} and the specific number
refers to the sheet and line number in
\href{xv6-src-booklet.pdf}{xv6-src-booklet.pdf}, as in the Lions'
Commentary on UNIX 6th Edition~\cite{lions}. A good practice is to try
to read the source code first on your own (in your favorite
development environment, on github, or in a PDF viewer) and then come
back to this book.  By the end of this book you should be able to
understand every line of xv6 source code without having to consult this
book.
...
int uptime() & Return the number of clock ticks since boot. \\
...
int sync() & Wait until earlier file system modifications are on disk. \\
\end{tabular}
...
process's memory: \lstinline{fork} copies the instructions, data, and
...
\end{lstlisting}
Although the child starts with a copy of the parent's memory, the
parent and child execute with separate memory and separate registers:
...
copy-on-write (see Chapter~\ref{CH:PGFAULTS}).
...
closing and re-opening chosen file descriptors in the child,
