...
exit(n) & Terminate the current process and return n to parent \\
wait(xstatus) & Wait for a child process to exit and receive its exit status \\
...
to release resources such as memory and open files.  The calling
process passes an exit status to
\lstinline{exit} that the kernel passes to the parent.  The convention is
to use the exit status to indicate if the child completed successfully (in
which case it returns 0) or if it failed to complete (in which case it
returns a -1).
The
\indexcode{wait}
system call returns the pid of an exited child of the
current process and its exit status; if none of the caller's children
has exited,
\indexcode{wait}
waits for one to do so.
If the parent doesn't care about the exit status of a child, it can
pass 0 (a null pointer) to
\lstinline{wait}.
