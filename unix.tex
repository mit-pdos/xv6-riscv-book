...
\textit{system call}\index{system call}.
...
\begin{figure}[t]
\center
\includegraphics[scale=0.5]{fig/os.eps}
...
\lstinline{pid}\index{pid@\lstinline{pid}},
with each process.
...
\textit{child process}\index{child process}, 
with exactly the same memory contents
as the calling process, called the 
\textit{parent process}\index{parent process}.
\lstinline{Fork}
...
\lstinline{printf}\index{printf@\lstinline{printf}}
call first.
...
\lstinline{getcmd}\index{getcmd@\lstinline{getcmd}}.
Then it calls 
\lstinline{fork}, 
...
\lstinline{malloc}\index{malloc@\lstinline{malloc}})
can call
\lstinline{sbrk(n)}
...
A 
\textit{file descriptor}\index{file descriptor} 
...
\textit{root}\index{root}.
A 
\textit{path}\index{path} 
like
\lstinline{/a/b/c}
...
\textit{current directory}\index{current directory},
