...
\lineref{kernel/proc.c:/^sleep.void./} and \indexcode{wakeup}
...
\section{Code: Pipes}
%% 
xv6's pipes are an example of code that sleeps
to synchronize producers and
consumers.
...
\lstinline{wakeup} marks that process as
\indexcode{RUNNABLE}.
...
and
\lstinline{&pi->nwrite});
this might make the system more efficient in the 
...
%% 
An interesting example of sleeping, introduced in Chapter~\ref{CH:UNIX},
...
\lstinline{kkill}
sets its state to \lstinline{RUNNABLE}
...
The virtio driver
\lineref{kernel/virtio\_disk.c:/while..b->disk.==.1/} 
...
\item It prevents races between \lstinline{sleep} and \lstinline{wakeup}.
