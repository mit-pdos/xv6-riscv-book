...
A
.italic driver
is the piece of code in an operating system that manage a particular
device: provides interrupt handlers for them, causes them to generate interrupts, etc.
Drivers can be tricky code because a driver executes concurrently with the
devices that it manages, but they must coordinate their actions.
.PP
...
disk as a numbered sequence of 512-byte blocks (also called sectors): sector 0
is the first 512 bytes, sector 1 is the next, and so on.  To represent disk
