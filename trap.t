...
the kernel instead of the next process instruction.  These events include when a
I/O device wants attention, the user program does some illegal (e.g., references
a virtual address for which there is no PTE), or when the user program wants to
ask the kernel for a service with a system call.  There are three main
challenges in handling these events: 1) the kernel must arrange that a processor
switches from user mode and back correctly to ensure proper isolation between
kernel and user processes; 2) the kernel and devices operate concurrently and
the kernel and the device must coordinate their actions; and 3) addressing these
two challenges these requires detailed understanding of the hardware and can
...
.\"
With a system call a user program can ask for an
operating system service, as we saw at the end of the last chapter.  
The term
.italic-index exception
refers to an illegal program
action that generates an interrupt.  Examples of illegal programs actions
include divide by zero, attempt to access memory for a PTE that is not present,
and so on. 
The term
.italic-index interrupt
refers to an signal generated by hardware devices that
...
A
.italic-index driver
is the piece of code in an operating system that manage a particular device:
provides interrupt handlers for them, causes them to generate interrupts, etc.
Drivers can be tricky code because a driver executes concurrently with the
devices that it manages, but they must coordinate their actions.  In addition,
write the driver one must understand the device (e.g., which I/O ports do what),
and devices can be complex and are often poorly documented.
.PP
...
out.  The driver and device must follow a protocol so that they know in what
state a sector is.
.\"
.\" -------------------------------------------
.\"
