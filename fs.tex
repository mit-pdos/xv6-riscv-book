...
At this point please read the files \lstinline{kernel/buf.h}
\fileref{kernel/buf.h}, \lstinline{kernel/fs.h} \fileref{kernel/fs.h},
\lstinline{kernel/fs.c} \fileref{kernel/fs.c},
\lstinline{kernel/sysfile.c} \fileref{kernel/sysfile.c}, and
\lstinline{kernel/file.c} \fileref{kernel/file.c}.
...
block by calling \indexcode{log_write} \lineref{kernel/log.c:/^log_write/}, which is a proxy for
\lstinline{bwrite} and only the logging layer
calls \lstinline{bwrite};  2)
...
At this point please read the files \lstinline{kernel/bio.c}
\fileref{kernel/bio.c} and
\lstinline{kernel/log.c} \fileref{kernel/log.c}.
...
directory)
\lineref{kernel/fs.c:/^..int.last.=/},
then the inode and its data blocks must
be freed.
\lstinline{iput}
calls
\indexcode{itrunc}
...
and then calls
\indexcode{ifree}
\lineref{kernel/fs.c:/^ifree/}
to set the inode's type to 0 (unallocated) on disk.
...
\lstinline{iput}
is freeing.
This can happen only after
\lstinline{ifree}
writes the disk so that the inode has type zero.
\lstinline{iput}
calls
\lstinline{ifree}
last, after it has truncated the inode, released the inode's
sleep-lock, and dropped its own reference.
So by the time the allocating thread can see that the inode is free,
\lstinline{iput}
is done with it, and the race is benign.
...
such file exists, then it can free those files.  This is the solution implemented
by xv6.  During boot,
\lstinline{fsinit}
calls
\indexcode{ireclaim}
\lineref{kernel/fs.c:/^ireclaim/},
which examines every on-disk inode, looking for ones that are marked
allocated (non-zero type) but have a link count of zero; these are
exactly the files that had been unlinked but were still open when
the crash occurred.
For each such orphan,
\lstinline{ireclaim} calls \lstinline{iput}
to free the inode and its data blocks.
...
Assuming 
\lstinline{old}
exists, is not a directory, and has fewer than
\lstinline{NLINK_MAX}
links
\linerefs{kernel/sysfile.c:/namei.old/,/NLINK_MAX/},
\lstinline{sys_link}
increments its 
\lstinline{ip->nlink}
count
\lineref{kernel/sysfile.c:/^..ip->nlink\+\+/}.
The limit exists because
\lstinline{nlink}
is a 16-bit field in the on-disk inode; without the check,
creating too many links would overflow the count.
Then
\lstinline{sys_link}
calls
\indexcode{nameiparent}
...
\lineref{kernel/sysfile.c:/if..dirlink\(dp/}.
\lstinline{create},
like
\indexcode{sys_link},
