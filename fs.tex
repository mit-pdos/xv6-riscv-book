...
\textit{persistence}\index{persistence}
so that data is still available after a reboot.

The xv6 file system provides Unix-like files, directories, and pathnames
(see Chapter~\ref{CH:UNIX}), and stores its data on an IDE disk for
persistence (see Chapter~\ref{CH:TRAP}). The file system addresses
several challenges:
\paragraph{\textbullet}The file system needs on-disk data structures to represent the tree
...
\paragraph{\textbullet}The file system must support
\textit{crash recovery}\index{crash recovery}.
...
\paragraph{\textbullet}Different processes may operate on the file system at the same time,
so the file system code must coordinate to maintain invariants.
\paragraph{\textbullet}Accessing a disk is orders of magnitude slower than accessing
...
\textit{transaction}\index{transaction},
...
\textit{superblock}\index{superblock}; 
...
\lstinline{mfks}\index{mfks@\lstinline{mfks}},
...
\lstinline{bread}\index{bread@\lstinline{bread}}
and
\lstinline{bwrite}\index{bwrite@\lstinline{bwrite}};
...
\lstinline{brelse}\index{brelse@\lstinline{brelse}}
...
\begin{figure}[t]
\center
\includegraphics[scale=0.5]{fig/fslayout.eps}
...
\lstinline{bcache.head}\index{bcache.head@\lstinline{bcache.head}},
not the
\lstinline{buf}
array.
...
calls
\lstinline{bget}\index{bget@\lstinline{bget}}
...
\textit{group commit}\index{group commit}.
...
\lstinline{write}\index{write@\lstinline{write}}
and
\lstinline{unlink}\index{unlink@\lstinline{unlink}}.
...
\lstinline{begin_op}\index{begin_op@\lstinline{begin_op}}
...
\lstinline{log_write}\index{log_write@\lstinline{log_write}}
...
\textit{absorption}\index{absorption}.
...
\lstinline{end_op}\index{end_op@\lstinline{end_op}}
...
\lstinline{install_trans}\index{install_trans@\lstinline{install_trans}}
...
\lstinline{recover_from_log}\index{recover_from_log@\lstinline{recover_from_log}}
\lineref{kernel/log.c:/^recover_from_log/}
is called from 
\lstinline{initlog}\index{initlog@\lstinline{initlog}}
...
\lstinline{filewrite}\index{filewrite@\lstinline{filewrite}}
...
the log.  The call to
\lstinline{writei}\index{writei@\lstinline{writei}}
...
\lstinline{balloc}\index{balloc@\lstinline{balloc}}
allocates a new disk block, and
\lstinline{bfree}\index{bfree@\lstinline{bfree}}
frees a block.
\lstinline{Balloc}
The loop in
\lstinline{balloc}
...
\lstinline{struct dinode}\index{struct dinode@\lstinline{struct dinode}}
...
\lstinline{struct inode}\index{struct inode@\lstinline{struct inode}}
...
The
\lstinline{iget}\index{iget@\lstinline{iget}}
and
\lstinline{iput}\index{iput@\lstinline{iput}}
...
\lstinline{ilock}\index{ilock@\lstinline{ilock}}.
...
xv6 calls
\lstinline{ialloc}\index{ialloc@\lstinline{ialloc}}
...
The function
\lstinline{iunlock}\index{iunlock@\lstinline{iunlock}}
...
\lstinline{Iput}
calls
\lstinline{itrunc}\index{itrunc@\lstinline{itrunc}}
...
array.
The first
\lstinline{NDIRECT}\index{NDIRECT@\lstinline{NDIRECT}}
blocks of data are listed in the first
\lstinline{NDIRECT}
entries in the array; these blocks are called 
\textit{direct blocks}\index{direct blocks}.
The next 
\lstinline{NINDIRECT}\index{NINDIRECT@\lstinline{NINDIRECT}}
blocks of data are listed not in the inode
but in a data block called the
\textit{indirect block}\index{indirect block}.
...
\lstinline{NDIRECT} 
\lstinline{x}
\lstinline{BSIZE}\index{BSIZE@\lstinline{BSIZE}})
...
The function
\lstinline{bmap}\index{bmap@\lstinline{bmap}}
manages the representation so that higher-level routines such as
\lstinline{readi}\index{readi@\lstinline{readi}}
and
\lstinline{writei}\index{writei@\lstinline{writei}},
...
\lstinline{ip->type}
\lstinline{==}
\lstinline{T_DEV}\index{T_DEV@\lstinline{T_DEV}}.
...
The function
\lstinline{dirlink}\index{dirlink@\lstinline{dirlink}}
...
\lstinline{inode}.
The function
\lstinline{nameiparent}\index{nameiparent@\lstinline{nameiparent}}
...
\lstinline{namex}\index{namex@\lstinline{namex}}
to do the real work.
...
Then it uses
\lstinline{skipelem}\index{skipelem@\lstinline{skipelem}}
...
\lstinline{struct file}\index{struct file@\lstinline{struct file}}
\lineref{kernel/file.h:/^struct.file/},
which is a wrapper around either an inode or a pipe,
plus an i/o offset.
Each call to 
\lstinline{open}\index{open@\lstinline{open}}
...
\lstinline{dup}\index{dup@\lstinline{dup}}
or shared it with a child using
\lstinline{fork}\index{fork@\lstinline{fork}}.
...
the 
\lstinline{ftable}\index{ftable@\lstinline{ftable}}.
The file table
has a function to allocate a file
(\lstinline{filealloc}\index{filealloc@\lstinline{filealloc}}),
create a duplicate reference
(\lstinline{filedup}\index{filedup@\lstinline{filedup}}),
release a reference
(\lstinline{fileclose}\index{fileclose@\lstinline{fileclose}}),
and read and write data
\lstinline{fileread}\index{fileread@\lstinline{fileread}} "" (
and 
\lstinline{filewrite}\index{filewrite@\lstinline{filewrite}}).
...
The functions
\lstinline{filestat}\index{filestat@\lstinline{filestat}},
\lstinline{fileread}\index{fileread@\lstinline{fileread}},
and
\lstinline{filewrite}\index{filewrite@\lstinline{filewrite}}
implement the 
\lstinline{stat}\index{stat@\lstinline{stat}},
\lstinline{read}\index{read@\lstinline{read}},
and
\lstinline{write}\index{write@\lstinline{write}}
operations on files.
\lstinline{Filestat}
...
The functions
\lstinline{sys_link}\index{sys_link@\lstinline{sys_link}}
and
\lstinline{sys_unlink}\index{sys_unlink@\lstinline{sys_unlink}}
...
The function
\lstinline{create}\index{create@\lstinline{create}}
...
\lstinline{open}\index{open@\lstinline{open}}
with the
\lstinline{O_CREATE}\index{O_CREATE@\lstinline{O_CREATE}}
flag makes a new ordinary file,
\lstinline{mkdir}\index{mkdir@\lstinline{mkdir}}
makes a new directory,
and
\lstinline{mkdev}\index{mkdev@\lstinline{mkdev}}
...
\lstinline{type} "" (
\lstinline{==}
\lstinline{T_FILE}\index{T_FILE@\lstinline{T_FILE}})
...
\lstinline{sys_open}\index{sys_open@\lstinline{sys_open}},
\lstinline{sys_mkdir}\index{sys_mkdir@\lstinline{sys_mkdir}},
and
\lstinline{sys_mknod}\index{sys_mknod@\lstinline{sys_mknod}}.
\lstinline{Sys_open}
...
\lstinline{open}\index{open@\lstinline{open}}
is passed the
\lstinline{O}\index{O@\lstinline{O}}\_CREATE
flag, it calls
\lstinline{create}
\lineref{kernel/sysfile.c:/create.*T_FILE/}.
Otherwise, it calls
\lstinline{namei}\index{namei@\lstinline{namei}}
...
The function
\lstinline{sys_pipe}\index{sys_pipe@\lstinline{sys_pipe}}
...
\lstinline{fsck}\index{fsck@\lstinline{fsck}}
