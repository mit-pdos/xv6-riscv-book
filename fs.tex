...
focusing on the first two challenges, while Chapter~\ref{CH:LOG}
focuses on the last two challenges.
...
The top layers of the file system, every layer above logging, are
largely independent of the logging layers.  The top layers are concerned with
implementing on-disk data structures to represent directories
and files.  The bottom layers are concerned with implementing
transactions to allow the file system to atomically perform updates that involve multiple
disk blocks.  The rest of this chapter describes each layer,
except for the logging layer, which is presented in 
Chapter~\ref{CH:LOG}.
