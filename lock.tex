...
f() {
  acquire(&lock);
  if(data == 0){
    this_function_should_only_be_called_once();
    h();
    data = 1;
  }
  release(&lock);
...
\lstinline{this_function_should_only_be_called_once} will be called
...
\lstinline{g}, \lstinline{this_function_should_only_be_called_once}
...
\lstinline{this_function_should_only_be_called_once}, a deadlock is
...
\lstinline{this_function_should_only_be_called_once} twice may
...
in similar style to \lstinline{push_off}.
