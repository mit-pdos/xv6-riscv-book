...
the data if the interrupt occurs just as the interrupted code is
...
for lots of concurrency, since it can increase performance
and responsiveness. However, they must then
...
This chapter focuses on a widely
used concurrency control technique: the \indextext{lock}.
As a preview, here's what use of a lock in the xv6 kernel looks like:
\begin{lstlisting}
struct spinlock lk;
...;
acquire(&lk);
...
say that the lock protects the data item.  Although a lock is an
easy-to-understand concurrency control mechanism, 
it can limit performance by serializing
concurrent operations on the data it protects.
...
list of free pages: \lstinline{kalloc()} \lineref{kernel/kalloc.c:/^kalloc/} pops a
page of memory from the list, and \lstinline{kfree()}
\lineref{kernel/kalloc.c:/^kfree/} pushes a page onto the list.
...
\lstinline{push}
at the same time,
both might execute line~\ref{line:next} as shown in Figure~\ref{fig:smp},
...
The lock is said to be protecting
\lstinline{list}.
...
Please read \lstinline{kernel/spinlock.h} \fileref{kernel/spinlock.h} and \lstinline{kernel/spinlock.c} \fileref{kernel/spinlock.c}.
...
Were there no concurrency, xv6 could acquire a lock with code like
...
guarantee mutual exclusion in the face of concurrency.
Two CPUs could simultaneously
...
\lstinline{amoswap r, r, a}.
The
\lstinline{amoswap}
instruction
reads the value at memory address {\tt a},
writes the contents of register {\tt r} to that address,
and puts the value it read into {\tt r}.
That is, it swaps the contents of the register and the addressed
memory location.
...
The C compiler provides a family of ``atomic'' built-in functions
that generate atomic instructions. Xv6's \lstinline{acquire()}
uses \lstinline{__atomic_exchange_n(addr, value, ...)}, which boils down to
the 
\lstinline{amoswap}
instruction;
the function returns the old (swapped) contents of {\tt *addr}.
Here's a good way to write the loop in {\tt acquire}:
...
uses the above loop (Section~\ref{SEC:MEMORDER} will explain \lstinline{__ATOMIC_ACQUIRE}).
...
Conceptually, the release just assigns zero to
\lstinline{lk->locked}.
...
Instead,
\lstinline{release}
uses the C atomic function
\lstinline{__atomic_store_n(addr, 0, ...)}
that stores zero to the address in a single
...
First, in any code that might cause a memory location to be
written by one CPU 
at the same time that another CPU reads or writes it,
...
kernel can do this on a multiprocessor by having a single lock;
...
Many uniprocessor operating systems have been converted to
...
CPU can execute in the kernel at a time.  If the kernel consumes
significant CPU time, more parallelism could be obtained by
protecting different objects or modules with different locks,
so that different CPUs could be
executing in different parts of the kernel at the same time.
...
Ultimately lock granularity decisions need to be driven by 
performance requirements as well as complexity considerations.
...
in their second calls to {\tt acquire()}, since both locks are already
...
\lstinline{sleep}/\lstinline{wakeup}
mechanism
works (see Chapter~\ref{CH:SLEEP}).
For example,
...
re-acquire a lock it already holds are called a {\it re-entrant} or {\it
...
holds a lock from re-acquiring it. Detecting this situation is the
purpose of the call to {\tt holding} in {\tt acquire}.
...
\lstinline{push_off} and \lstinline{pop_off} tell the CPU to enable
and disable interrupts by changing the \lstinline{SIE}
(Supervisor Interrupt Enable)
bit in the \lstinline{sstatus} (Supervisor Status)
register, using \lstinline{intr_on} and \lstinline{rc_sstatus(SSTATUS_SIE)};
the latter clears the \lstinline{SIE} bit.
...
\lstinline{lk->locked}
\lineref{kernel/spinlock.c:/while..__atomic_exchange_n/}.
...
only after 
releasing the lock
\lineref{kernel/spinlock.c:/^..__atomic_store_n/}.
...
code, but is not what actually happens.
...
Memory access re-ordering by the CPU and compiler can cause
problems when
...
The good news is that compilers and CPUs provide ways
for programmers to prevent re-ordering.
These appear in xv6 as
the \lstinline{__ATOMIC_ACQUIRE} and \lstinline{__ATOMIC_RELEASE}
arguments to the C atomic functions
used in \lstinline{acquire} and \lstinline{release}.
Each has two effects.
First, each tells the compiler not to move memory references
past the atomic call.
Second, each tells the compiler to generate instructions
that tell the CPU not to move execution-time memory references past the
instructions generated for the atomic call.
Such a ``don't move past'' point is called
a \indextext{memory barrier} or 
a \indextext{fence}.

The barriers in xv6's spinlock functions have two important
effects on code that uses locks. First, all memory references
(reads and writes) that occur in the C code 
before a \lstinline{release()}
are guaranteed to take effect on the memory system
before the \lstinline{release()}
completes. This includes
both memory references in the critical section and memory
references before the critical section. Second, all memory
references in the C code after the \lstinline{acquire}
are guaranteed {\bf not} to take effect before the successful
completion of the \lstinline{amoswap} in \lstinline{acquire()}.
Again, this includes both references inside the critical
section and references after the \lstinline{release}.

These barriers produce an important transitive effect. For example, if
a CPU writes to a newly-allocated data object (without holding locks)
and then, in a critical section, links that object into a list, any
CPU that subsequently acquires a lock and obtains a pointer to the
object from the list is guaranteed to observe the first CPU's writes.
...
\section{Programming with Atomics}

Though xv6 doesn't do this, it is sometimes possible to update shared
data while avoiding races by direct use of atomic instructions,
without use of locks. Most CPUs conveniently supply dozens of atomic
instructions, typically with the pattern that they read a memory
location, modify the value, and write the new value to the same memory
location, atomically. The RISC-V \lstinline{amoadd} instruction is an
example: it atomically adds a register's contents to a location in
memory. The C compiler provides ``atomic'' wrappers for
these instructions.\footnote{\url{https://gcc.gnu.org/onlinedocs/gcc/_005f_005fatomic-Builtins.html}}
Thus code could increment a shared counter without locks using
\begin{lstlisting}
  __atomic_fetch_add(&counter, 1, __ATOMIC_SEQ_CST);
\end{lstlisting}
It can be useful to avoid locks because they consume
CPU time, or because a structural reason such as deadlock
avoidance makes use of locks awkward.
...
\lstinline{locked}
field that is protected by a spinlock.
If the sleep-lock is held,
\lstinline{acquiresleep}
releases the spinlock and calls
\lstinline{sleep},
which yields the CPU until
\lstinline{releasesleep}
calls
\lstinline{wakeup}.
...
% LocalWords:  usertests SIE sstatus rc amoadd register's SEQ sched
% LocalWords:  tex
