...
As an example of why we need locks, consider a linked list accessible
from any CPU on a multiprocessor.  Figure~\ref{fig:smp} illustrates
the setting: the linked list is in a memory that is shared among
several processors, which manipulate the linked list using load and
store instructions.  In reality, the processors have caches, but
conceptually the multiprocessor systems behaves as if it has a single,
shared memory.

\mfk{Need to talk more about parallelism first; we want to get
  speedup. easy when no shared state; hard when there is shared state
  (correctness and scalability are hard). maybe here or into of
  this chapter.}
...
It is possible to implement locks without atomic instructions \mfk{ref}, but it is
...
lock-free data structures and algorithms \mfk{rcu ref}.  For example, it is possible to
