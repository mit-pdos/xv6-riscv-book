...
First, they tell the compiler not to move memory references
past the atomic calls, so that instructions for memory
references that are inside the C critical section stay
between the \lstinline{amoswap} instructions that acquire and release the lock.
Second, they tell the compiler to generate instructions
that tell the CPU not to move memory references past those
acquire/release instructions either.

Such a ``don't move past'' point is called
is a \indextext{memory barrier} or 
\indextext{fence}.
