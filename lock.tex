...
{\tt wait} to cleanup their children's address spaces on two different
CPUs.  On each CPU, the kernel will call the kernel memory allocator
...
Ideally, we like to run the {\tt kfree}'s of the two parent processes
in parallel without any coordination between the two processes, but
this breaks correctness.

Figure~\ref{fig:smp} illustrates the setting in more detail: the
linked list is in a memory that is shared among two CPUs, which
manipulate the linked list using load and store instructions.  (In
reality, the processors have caches, but conceptually the
multiprocessor systems behaves as if it has a single, shared memory.)
...
little of that, but a sophisticated kernels, for example, re-organize
data structures in order to avoid lock contention.  In the list
