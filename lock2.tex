...
Often one sees locks around sequences of writes to sets of related
...
The answer is that the C language specification says that you
cannot conclude \textit{anything} about the effects of any
...
The only kind of guarantee C gives code that doesn't lock is that
it will yield the expected results if there is only one thread;
there's no guarantee that memory will have the content that
the code may seem to imply. Adding locks does two things: locks
prevent other threads from seeing intermediate state of whatever
optimized update strategy the compiler and CPU
choose, and locks tell the compiler and CPU to 
